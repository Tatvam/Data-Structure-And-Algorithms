...
int main(){
    ...
    int  j = 1;
    ll sum = 0;
    ll ans = h+v[0].second-v[0].first;
    for(int i = 1;i<n;i++){
        while(j<n && (v[j].first-v[j-1].second+sum)<h){
            sum+=v[j].first-v[j-1].second;
            j++;
        }
         if(ans<(h-sum+v[j-1].second-v[i-1].first)){
                ans = h-sum+v[j-1].second-v[i-1].first;
            }
